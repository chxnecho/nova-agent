...
async def test_team_full_pipeline(tmp_path):
    ...
    mock.enqueue(
        ...
        resp_text('[{"title": "write file", "detail": "create a.txt"},'
    ...
    for expected in ("plan_start", "plan_ready", "executor_start",
                     "executor_done", "critique", "done"):
...
async def test_team_revision_round(tmp_path):
    ...
    mock.enqueue(
        ...
        resp_text('{"verdict": "NEEDS_REVISION", "reasoning": "wrong",'
                  ' "feedback": "do it properly"}'),               # critic rejects
