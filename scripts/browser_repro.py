...
async def main(port):
    async with async_playwright() as p:
        ...
        try:
            ...
            browser = await p.firefox.launch(executable_path="/usr/bin/firefox", **launch_kwargs)
        ...
        await page.wait_for_function(
            "document.getElementById('verInfo').textContent.includes('v9')", timeout=15000
        )
        ...
        while time.monotonic() - t0 < 90:
            ...
            if state["len"] != last_len or state["busy"] != last_busy:
                ...
                print(
                    f"t={t:6.1f}s  正文={state['len']:4d}字  工具={state['tools']}  "
        ...
        incremental = sum(
            1 for i in range(1, len(samples)) if samples[i][1] > samples[i - 1][1] > 0
        )
        ...
        print(
            ...
            "✅ 实时逐字更新"
            if incremental >= 3
            else ("❌ 全部内容最后一次性出现" if incremental <= 1 else "⚠️ 更新不连贯"),
        )
        ...
        print(
            "完成后思考面板折叠:",
            "✅" if collapsed else ("(无面板)" if collapsed is None else "❌"),
        )
